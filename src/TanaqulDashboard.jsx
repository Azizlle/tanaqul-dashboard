...
const BADGE_AR = {
  ...
  BILLED:"مُفوتر", OVERDUE:"متأخر", LIMIT:"محدد", MARKET:"سوق", GTC:"ساري حتى الإلغاء",
  Gold:"ذهب", Silver:"فضة", Platinum:"بلاتين",
};
            ...
            <div key={p.symbol} style={{
              ...
              <div>
                ...
                <p style={{fontSize:26,fontWeight:800,color:C.navy,lineHeight:1,display:"flex",alignItems:"center",gap:6,direction:"ltr",unicodeBidi:"embed"}}>
                  ...
                  {(p.priceSAR||0).toLocaleString("en-SA",{minimumFractionDigits:2,maximumFractionDigits:2})}
                </p>
...
// ─── Shared initial order book data — loaded from API ───
      ...
      {spread ? (
        <div style={{display:"flex",gap:12,marginBottom:12,padding:"8px 12px",borderRadius:8,background:"#FAF8F5",border:`1px solid ${C.border}`}}>
          <div style={{textAlign:"center",flex:1}}>
            ...
            <p style={{fontSize:16,fontWeight:800,color:C.greenSolid}}>SAR {(bestBid||0).toFixed(2)}</p>
          ...
          <div style={{textAlign:"center",flex:1}}>
            ...
            <p style={{fontSize:16,fontWeight:800,color:"#C85C3E"}}>SAR {(bestAsk||0).toFixed(2)}</p>
...
const Dashboard = () => {
  ...
  const volumeToday = todayMatches.reduce((a,m)=>a+(Number(m.totalSAR)||0),0);
  const commToday   = todayMatches.reduce((a,m)=>a+(Number(m.commission)||0),0);
  const adminToday  = todayMatches.reduce((a,m)=>a+(Number(m.adminFee)||0),0);
  const volumeAll   = matches.reduce((a,m)=>a+(Number(m.totalSAR)||0),0);
  ...
  // 7-day volume sparkline data — compute from actual matches per day
  const spark7 = (() => {
    const days = [];
    for (let i = 6; i >= 0; i--) {
      const d = new Date(); d.setDate(d.getDate() - i);
      const ds = d.toISOString().slice(0,10);
      days.push(matches.filter(m=>m.date&&m.date.startsWith(ds)).reduce((a,m)=>a+(Number(m.totalSAR)||0),0));
    }
    return days;
  })();
    ...
    <div>
      ...
      <div style={{background:`linear-gradient(135deg, ${C.sidebar} 0%, #3D3225 60%, ${C.gold}22 100%)`,borderRadius:18,padding:"24px 30px",marginBottom:20,display:"flex",justifyContent:"space-between",alignItems:"center",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{position:"relative",zIndex:1}}>
          <h2 style={{fontSize:22,fontWeight:800,color:"#FFF",margin:0}}>{isAr?`مرحباً بعودتك، ${adminName||"مشرف"} 👋`:`Welcome back, ${adminName||"Admin"} 👋`}</h2>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:14,marginBottom:28}}>
        ...
        <StatCard icon={Icons.fire(22,C.red)} title={isAr?"بانتظار الحرق":"Pending Burn"} value={(s.tokensPendingBurn||0).toLocaleString()} />
        <StatCard icon={Icons.block(22,C.navy)} title={isAr?"آخر كتلة":"Last Block"} value={"#"+(s.latest_block_number||s.blockNumber||0)} sub={s.lastBlock||""} gold />
      </div>
    </div>
  );
};
...
const Investors = () => {
  ...
  const confirmAction = async () => {
    ...
    try {
      ...
      setInvestors(prev => prev.map(i => i.id===sel.id ? {...i, status:newStatus} : i));
      showToast("✅ "+msgs[action]+" (offline mode)");
    }
...
<div style={{display:"flex",gap:10,marginBottom:8,flexWrap:"wrap",alignItems:"center"}}>
      ...
      {action==="notify"&&sel&&<Modal title={(isAr?"إشعار — ":"Notify — ")+sel.nameEn} onClose={()=>{setSel(null);setAction(null);}}>
        ...
        <div style={{display:"flex",gap:8}}>
          <Btn variant="teal" onClick={()=>{if(!notifyMsg.trim()){showToast(isAr?"⚠️ الرسالة فارغة":"⚠️ Message is empty");return;}showToast("✅ Notification sent to "+sel.nameEn);setSel(null);setAction(null);}}>{isAr?"إرسال إشعار":"Send Notification"}</Btn>
          <Btn variant="outline" onClick={()=>{setSel(null);setAction(null);}}>{t("Cancel")}</Btn>
        </div>
      </Modal>}
...
const TransactionLog = () => {
  ...
  const liveTxRows = matches.map(m=>({id:m.id,investor:m.buyerName||m.filledFor,buyerName:m.buyerName||"",sellerName:m.sellerName||"",buyerNationalId:m.buyerNid||"",sellerNationalId:m.sellerNid||"",type:"MATCH",metal:m.metal,metalAmt:String(m.totalSAR),commission:String(m.commission),adminFee:String(m.adminFee||0),method:"Wallet",total:String(m.totalSAR),status:"COMPLETED",date:m.date}));
  ...
  const exportCSV = () => {
    ...
    const csvRows = [headers.join(","), ...rows.map(r=>[r.id,r.buyerName||"",r.buyerNationalId||r.buyerNid||"",r.sellerName||"",r.sellerNationalId||r.sellerNid||"",r.type,r.metal,r.metalAmt,r.commission,r.adminFee,r.total,r.method,r.status,r.date].map(v=>`"${sanitize(v)}"`).join(","))];
...
const Appointments = () => {
  ...
  // OTP is verified server-side via /appointments/:id/otp/verify
    ...
    if(row.status==="RESCHEDULED"||row.status==="BOOKED") {
      if(expired) return (
        <div style={{display:"flex",gap:4,justifyContent:"center"}}>
          <Btn small variant="teal" onClick={()=>{setSel(row);setModal("start");}}>{inProgress.has(row.id)?(isAr?"متابعة":"Continue"):(isAr?"بدء":"Start")}</Btn>
        ...
        <div style={{textAlign:"center",padding:"8px 0 16px"}}>
          ...
          <div style={{display:"flex",gap:8,justifyContent:"center"}}>
            <Btn variant="gold" onClick={async ()=>{
              ...
              setAppointments(prev=>prev.map(a=>a.id===sel.id?{...a,status:"COMPLETED"}:a));
              ...
              if(sel.type==="DEPOSIT"&&startData.barcode){
                ...
                if(existingBar&&existingBar.status==="LEFT"){
                  ...
                  const newBar={id:"BAR-TMP-"+Date.now(),metal:sel.metal,weight:sel.quantity||sel.qty||"",barcode:startData.barcode.trim(),manufacturer,vault:(sel.vault||"").replace(" Vault 1","").replace(" Vault",""),status:"LINKED",depositor:sel.nationalId||sel.investorId||"—",deposited:new Date().toISOString().slice(0,10)};
...
const Financials = () => {
  ...
  useEffect(()=>{
    if(tab==="STORAGE FEES"){
      setSfLoading(true);
      apiFetch("/storage-fees/history").then(r=>r&&r.ok?r.json():null).then(d=>{
        if(d?.items) setStorageFeeHistory(d.items);
        else if(Array.isArray(d)) setStorageFeeHistory(d);
  ...
  const txRows = matches.map(m=>({id:m.id,investor:m.filledFor,type:"MATCH",metal:m.metal,metalAmt:String(m.totalSAR),commission:String(m.commission),adminFee:String(m.adminFee||0),method:"Wallet",total:String(m.totalSAR),status:"COMPLETED",date:m.date})).concat(matches.length > 0 ? [] : []);
  ...
  const volToday  = todayM.reduce((a,m)=>a+(Number(m.totalSAR)||0),0);
  const commToday = todayM.reduce((a,m)=>a+(Number(m.commission)||0),0);
  const adminToday= todayM.reduce((a,m)=>a+(Number(m.adminFee)||0),0);
  ...
  const confirmWithdrawal = async () => {
    ...
    try { const wr=await apiFetch("/withdrawals?page=1&page_size=9999"); if(wr&&wr.ok){const wd=await wr.json();const items=wd.items||wd.withdrawals||[];if(Array.isArray(items)&&items.length>0)setWithdrawals(items.map(w=>({id:w.display_id||w.id,_uuid:String(w.id),nationalId:w.national_id||"",amount:String(w.amount||0),investor:w.investor_name||w.national_id||"",bank:w.bank_info||"",iban:w.iban||"",status:w.status||"PENDING",requested:w.requested_at||"",processed:w.processed_at||"",rejectReason:w.reject_reason||""})));} } catch(e){}
      ...
      {tab==="STORAGE FEES"&&<>
        ...
        <TTable cols={[
          ...
          {key:"id",label:isAr?"الإجراءات":"Actions",render:(_,row)=>(
            ...
            investor: r.investor_name || r.investor_id || "—",
...
const Reports = () => {
  ...
  const totalComm   = matches.reduce((a,m)=>a+(Number(m.commission)||0),0);
  const totalAdmin  = matches.reduce((a,m)=>a+(Number(m.adminFee)||0),0);
  const volGold     = matches.filter(m=>m.metal==="Gold").reduce((a,m)=>a+(Number(m.totalSAR)||0),0);
  const volSilver   = matches.filter(m=>m.metal==="Silver").reduce((a,m)=>a+(Number(m.totalSAR)||0),0);
  ...
  const barsGold    = bars.filter(b=>b.metal==="Gold").length;
  const barsSilver  = bars.filter(b=>b.metal==="Silver").length;
  const barsPlat    = bars.filter(b=>b.metal==="Platinum").length;
  const barsTotal   = bars.length;
  const invActive   = investors.filter(i=>i.status==="ACTIVE").length;
  const invSusp     = investors.filter(i=>i.status==="SUSPENDED").length;
  const invBanned   = investors.filter(i=>i.status==="BANNED").length;
  const invTotal    = investors.length;
  const topInv      = [...investors].sort((a,b)=>(Number(b.holdingsValue)||0)-(Number(a.holdingsValue)||0));
  const apptDeposit = appointments.filter(a=>a.type==="DEPOSIT").length;
  const apptWithdr  = appointments.filter(a=>a.type==="WITHDRAWAL").length;
  const apptNoShow  = appointments.filter(a=>a.status==="NO_SHOW").length;
  const apptDone    = appointments.filter(a=>a.status==="COMPLETED").length;
  const apptTotal   = appointments.length;
  const noShowRate  = apptTotal>0 ? Math.round(apptNoShow/apptTotal*100) : 0;
  const complRate   = apptTotal>0 ? Math.round(apptDone/apptTotal*100) : 0;
  const minted      = appBlockStats?.tokensMinted||appDashStats?.tokensMinted||0;
  const burned      = appBlockStats?.tokensBurned||appDashStats?.tokensPendingBurn||0;
  ...
  return (
    <div>
      ...
      {editRow&&<Modal title={(isAr?"تعديل — ":"Edit — ")+editRow.id} onClose={()=>setEditRow(null)}>
        ...
        <div style={{display:"flex",gap:8}}>
          <Btn variant="gold" onClick={async ()=>{
            try{const uid=editRow._uuid||editRow.id;await apiFetch("/blacklist/"+uid,{method:"PATCH",body:JSON.stringify({reason:editRow.reason})});}catch(e){}
...
const ValidatorsTab = () => {
  ...
  const statusPill=(status)=>{
    ...
    const labAr={ACTIVE:"نشط",INACTIVE:"غير نشط"};
...
const Blocks = () => {
  ...
  const createGenesis = async () => {
    ...
    try{
      const r=await apiFetch("/blocks",{method:"POST"});
      if(r&&r.ok){const d=await r.json();alert("Genesis Block Created!\nHash: "+(d.hash||""));
        // Refresh blocks
        const br=await apiFetch("/blocks?page=1&page_size=9999");if(br&&br.ok){const bd=await br.json();const blist=bd.blocks||bd.items||bd;if(Array.isArray(blist))setAppBlocks(blist.map(b=>({number:b.number,hash:b.hash||"",txCount:b.tx_count||0,commission:String(b.commission||0),tanaqulShare:String(b.tanaqul_share||0),creatorShare:String(b.creator_share||0),validatorsShare:String(b.validators_share||0),validator:b.validator_name||"Tanaqul",timestamp:b.created_at||"",size:b.size_bytes?(Number(b.size_bytes)/1048576).toFixed(2)+" MB":"0 MB"})));}
  ...
  return (
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(6,1fr)",gap:14,marginBottom:18}}>
        ...
        <StatCard icon={Icons.token(22,C.teal)} title={isAr?"الرموز المصكوكة":"Tokens Minted"} value={appBlockStats?.tokensMinted || 0} />
        <StatCard icon={Icons.fire(22,"#C85C3E")} title={isAr?"الرموز المحروقة":"Tokens Burned"} value={appBlockStats?.commission_breakdown?.tanaqul ? (Number(appBlockStats.commission_breakdown.tanaqul)||0).toLocaleString("en-SA",{maximumFractionDigits:0}) : "0"} />
        <StatCard icon={Icons.aum(22,C.gold)} title={isAr?"العمولات الموزعة":"Commission Distributed"} value={<SARAmount amount={appBlockStats?.total_commission ? Number(appBlockStats.total_commission).toLocaleString("en-SA",{maximumFractionDigits:0}) : matches.reduce((a,m)=>a+(Number(m.commission)||0),0).toLocaleString("en-SA",{maximumFractionDigits:0})}/>} gold />
...
const AuditLog = () => {
  ...
  const generateCMANotif = (alert) => {
    ...
    const narrativeEn =
      ...
      (p ? `Trading profile: SAR ${(p.totalVolume||0).toLocaleString()} total volume, ${p.ordCount} orders placed, ${p.txCount} executed transactions. `:"") +
  ...
  const generateSAR = (alert) => {
    ...
    const narrativeEn =
      ...
      (p ? "Total trading volume: SAR "+(p.totalVolume||0).toLocaleString()+" across "+p.txCount+" transactions since account opening on "+p.joinDate+". ":"") +
    ...
    const narrativeAr =
      ...
      (p ? "إجمالي حجم تداول الشخص المعني "+(p.totalVolume||0).toLocaleString()+" ريال سعودي عبر "+p.txCount+" معاملة منذ فتح الحساب في "+p.joinDate+". ":"") +
            ...
            <tbody>
              {riskScores.map((r,i)=>(
                <tr key={r.nid} style={{borderBottom:`1px solid ${C.border}`,background:i%2===0?C.white:"#FAF8F5"}}>
                  ...
                  <td style={{padding:"10px 12px",fontWeight:600,color:C.navy,fontSize:13}}>SAR {(r.totalVolume||0).toLocaleString()}</td>
                    ...
                    <div style={{maxHeight:150,overflowY:"auto",display:"flex",flexDirection:"column",gap:4}}>
                      {[...task.log].reverse().map((entry,i)=>(
                        <div key={i} style={{display:"flex",gap:8,fontSize:13,color:C.text,padding:"4px 8px",background:C.white,borderRadius:6,border:`1px solid ${C.border}`}}>
                          <span style={{color:C.textMuted,flexShrink:0,fontFamily:"monospace",fontSize:12}}>{(entry.date?new Date(entry.date).toLocaleString("en-SA",{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}):"—")}</span>
        ...
        <div style={{padding:"4px 0"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
            ...
            <div style={{background:"#F5F0E8",borderRadius:8,padding:"8px 12px"}}>
              ...
              <p style={{fontSize:14,fontWeight:700,color:C.navy}}>{(amlModal.automatedAt ? new Date(amlModal.automatedAt).toLocaleString() : "—")}</p>
            </div>
          </div>
          <div style={{display:"flex",gap:8}}>
      ...
      <G title={isAr?"العمولة لكل طرف":"Commission Per Party"}>
        <div style={{background:C.purpleBg,borderRadius:10,padding:"10px 14px",marginBottom:14,border:"1px solid #C8D6E8"}}>
          <p style={{fontSize:14,color:C.blueSolid,fontWeight:500,lineHeight:1.6}}>
            {isAr?"تُفرض العمولة بشكل مستقل على كلا طرفي كل صفقة. يدفع المشتري قيمة الصفقة + عمولة المشتري. يستلم البائع قيمة الصفقة − عمولة البائع.":"Commission is charged independently on both sides of every trade. The buyer pays trade value + buyer commission. The seller receives trade value − seller commission."}
          </p>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14}}>
          <div>
            <label style={{display:"block",fontSize:13,fontWeight:700,color:C.textMuted,marginBottom:6}}>{isAr?"عمولة المشتري (%)":"BUYER COMMISSION (%)"}</label>
          ...
          <div>
            <label style={{display:"block",fontSize:13,fontWeight:700,color:C.textMuted,marginBottom:6}}>{isAr?"عمولة البائع (%)":"SELLER COMMISSION (%)"}</label>
        ...
        <div style={{background:C.navyDark,borderRadius:12,padding:"14px 18px",marginBottom:4}}>
          <p style={{fontSize:12,fontWeight:700,color:"#A89880",letterSpacing:"0.08em",marginBottom:12}}>{isAr?"مثال حي — صفقة 10,000 ريال":"LIVE EXAMPLE — 10,000 SAR TRADE"}</p>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
            ...
            <div style={{background:"rgba(255,255,255,0.05)",borderRadius:10,padding:"12px 14px",border:"1px solid rgba(255,255,255,0.08)"}}>
              ...
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
                <span style={{fontSize:14,color:"#A89880"}}>{isAr?"العمولة":"Commission"} ({commBuyer}%)</span>
                <span style={{fontSize:14,color:"#FCA5A5",fontWeight:600}}>+ {(buyComm||0).toLocaleString()} SAR</span>
              </div>
              <div style={{borderTop:"1px solid rgba(255,255,255,0.1)",paddingTop:8,display:"flex",justifyContent:"space-between"}}>
                ...
                <span style={{fontSize:17,color:C.gold,fontWeight:800}}>{(buyerPays||0).toLocaleString()} SAR</span>
              </div>
            </div>
            {/* Seller */}
            <div style={{background:"rgba(255,255,255,0.05)",borderRadius:10,padding:"12px 14px",border:"1px solid rgba(255,255,255,0.08)"}}>
              ...
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
                <span style={{fontSize:14,color:"#A89880"}}>{isAr?"العمولة":"Commission"} ({commSeller}%)</span>
                <span style={{fontSize:14,color:"#FCA5A5",fontWeight:600}}>− {(sellComm||0).toLocaleString()} SAR</span>
              </div>
              <div style={{borderTop:"1px solid rgba(255,255,255,0.1)",paddingTop:8,display:"flex",justifyContent:"space-between"}}>
                <span style={{fontSize:15,color:C.white,fontWeight:700}}>{isAr?"صافي الأرباح":"Net earnings"}</span>
                <span style={{fontSize:17,color:"#4ADE80",fontWeight:800}}>{(sellerGets||0).toLocaleString()} SAR</span>
              </div>
            </div>
          </div>
          <div style={{background:"rgba(212,160,23,0.12)",borderRadius:8,padding:"8px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <span style={{fontSize:14,color:"#A89880"}}>{isAr?"إجمالي عمولة المنصة المحصّلة":"Total platform commission collected"}</span>
            <span style={{fontSize:17,color:C.gold,fontWeight:800}}>{(totalComm||0).toLocaleString()} SAR</span>
          </div>
        </div>
      </G>
...
const OrderBook = () => {
  ...
  const issueRefund = (order, filledQty, execSAR, reason) => {
    ...
    if(order.nationalId === "SYSTEM" || order.nationalId === "7031990530") return; // Skip refund for system orders (Market Maker, Stabilizer)
  ...
  const [synLog,   setSynLog]   = useState([]); // populated from API orders filtered to synthetic
  ...
  useEffect(()=>{
    ...
    const synBuy = {id:"SYN-B-"+Date.now(), investor:"Takharoj Finance", investorAr:"تخارج المالية", nationalId:"7031990530", side:"BUY", metal:metal||"Gold", qty:synQty, filled:0, price:synBidPrice, payment:"Wallet", expiry:"GTC", status:"OPEN", placed:ts, synthetic:true, reason:`Spread ${spreadPct}% > ${maxSpreadPct}%`};
    const synSell = {id:"SYN-S-"+Date.now(), investor:"Takharoj Finance", investorAr:"تخارج المالية", nationalId:"7031990530", side:"SELL", metal:metal||"Gold", qty:synQty, filled:0, price:synAskPrice, payment:"Wallet", expiry:"GTC", status:"OPEN", placed:ts, synthetic:true, reason:`Spread ${spreadPct}% > ${maxSpreadPct}%`};
  ...
  const cancelOrder = async (id) => {
    ...
    try { const uid = order._uuid || order.id; await apiFetch("/orders/"+uid+"/cancel", {method:"POST"}); } catch(e) {}
    ...
    try {
      ...
      if(resp && resp.ok) {
        ...
        const [ordResp, matchResp] = await Promise.all([
          apiFetch("/orders?page=1&page_size=9999"),
          apiFetch("/matches?page=1&page_size=9999"),
        ]);
              ...
              {spread ? (
                <div style={{display:"flex",gap:16,marginBottom:12,padding:"8px 12px",borderRadius:8,background:spreadWide?"#FBF0EC":C.greenBg,border:`1px solid ${spreadWide?"#C85C3E44":C.gold+"44"}`}}>
                  <div style={{textAlign:"center"}}>
                    ...
                    <p style={{fontSize:17,fontWeight:700,color:C.greenSolid}}>SAR {(bestBid||0).toFixed(2)}</p>
                  </div>
                  <div style={{textAlign:"center",flex:1}}>
                  ...
                  <div style={{textAlign:"center"}}>
                    ...
                    <p style={{fontSize:17,fontWeight:700,color:"#C85C3E"}}>SAR {(bestAsk||0).toFixed(2)}</p>
                  </div>
                </div>
              ):(
                ...
                <div>
                  ...
                  {bids.length===0?<p style={{fontSize:13,color:C.textMuted,textAlign:"center"}}>—</p>:bids.map(b=>(
                    <div key={b.id} style={{display:"flex",justifyContent:"space-between",padding:"3px 6px",borderRadius:4,marginBottom:2,background:b.synthetic?"#F0EDF722":"#EFF5F244"}}>
                      <span style={{fontSize:13,color:C.greenSolid,fontWeight:600}}>{(b.price||0).toFixed(2)}</span>
                ...
                <div>
                  ...
                  {asks.length===0?<p style={{fontSize:13,color:C.textMuted,textAlign:"center"}}>—</p>:asks.map(a=>(
                    <div key={a.id} style={{display:"flex",justifyContent:"space-between",padding:"3px 6px",borderRadius:4,marginBottom:2,background:"#FBEAE544"}}>
                      <span style={{fontSize:13,color:"#C85C3E",fontWeight:600}}>{(a.price||0).toFixed(2)}</span>
      ...
      {tab==="matched" && (
        <div>
          <div style={{display:"flex",gap:12,marginBottom:20,flexWrap:"wrap"}}>
            ...
            <StatCard icon={Icons.aum(22,C.teal)} title={isAr?"إجمالي الحجم":"Total Volume"} value={<SARAmount amount={matches.reduce((s,m)=>s+(Number(m.totalSAR)||0),0).toLocaleString("en-SA")}/>} />
            <StatCard icon={Icons.commission(22,C.navy)} title={isAr?"إجمالي العمولة":"Commission"} value={<SARAmount amount={matches.reduce((s,m)=>s+(Number(m.commission)||0),0).toLocaleString("en-SA")}/>} />
      ...
      {tab==="stabilizer" && (
        <div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))",gap:16,marginBottom:24}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              <StatCard icon={Icons.aum(22,C.gold)} title={isAr?"\u0627\u0644\u062a\u0639\u0631\u0636 \u0627\u0644\u062d\u0627\u0644\u064a":"Current Exposure"} value={<SARAmount amount={(()=>{const exp=orders.filter(o=>o.synthetic&&(o.status==="OPEN"||o.status==="PARTIAL")).reduce((a,o)=>a+Math.round((o.qty-o.filled)*o.price),0);return (exp||0).toLocaleString("en-SA");})()}/>} sub={`${((orders.filter(o=>o.synthetic&&(o.status==="OPEN"||o.status==="PARTIAL")).reduce((a,o)=>a+Math.round((o.qty-o.filled)*o.price),0)/parseInt(stabCapSAR||1))*100).toFixed(1)}% of cap`} gold />
        ...
        <Modal title={isAr?"⚡ صانع السوق":"⚡ Market Maker"} onClose={()=>setMmOpen(false)}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            ...
            <div style={{padding:"8px 10px",borderRadius:8,background:C.bg,border:`1px solid ${C.border}`,display:"flex",flexDirection:"column",justifyContent:"center"}}>
              ...
              <p style={{fontSize:18,fontWeight:700,color:C.navy}}><SARAmount amount={(mmTotal||0).toLocaleString("en-SA",{minimumFractionDigits:2,maximumFractionDigits:2})}/></p>
            </div>
          </div>
...
const UserManagement = () => {
  ...
  useEffect(()=>{
    ...
    apiFetch("/audit?page=1&page_size=50").then(r=>r&&r.ok?r.json():null).then(d=>{
      if(d&&d.items){
...
const AccountProfile = () => {
  ...
  useEffect(()=>{
    ...
    apiFetch("/audit?page=1&page_size=200").then(r=>r&&r.ok?r.json():null).then(d=>{
      if(d&&d.items){
  ...
  const verifyOtp = async () => {
    try {
      const res = await apiFetch("/auth/otp/verify", {method:"POST", body:JSON.stringify({otp_code:phoneOtp.code, field:phoneOtp.field})});
      if(res && res.ok) {
      ...
      showToast(isAr?"❌ خطأ في التحقق":"❌ Verification error");
    }
  };
...
const GlobalSearch = ({ isOpen, onClose, setPage, setPageHint }) => {
  ...
  if(q.length >= 1) {
    ...
    // Messages search removed — comm center loads from API
  }
...
const InvestorTimeline = ({ investor, onClose }) => {
  ...
  // Transactions (fall back to matches if transactions array is empty)
  const txList = transactions.length > 0 ? transactions : matches.map(m=>({id:m.id,buyerNationalId:m.buyerNid||"",sellerNationalId:m.sellerNid||"",total:String(m.totalSAR),metal:m.metal,status:"COMPLETED",date:m.date,type:"MATCH",amount:m.totalSAR+"g",method:"—"}));
  txList.forEach(tx => {
  ...
  // Messages: fetched per-investor from audit log via API when needed
...
const CommCenter = () => {
  ...
  const sendMessage = async (to, body, channel, schedule) => {
    try {
      const res = await apiFetch("/sms/send", {method:"POST", body:JSON.stringify({to, message:body, channel, subject:compSubject, schedule_at:schedule||null})});
      if(res&&res.ok) {
    ...
    <div>
      ...
      {tab==="compose"&&<div style={{background:C.white,borderRadius:14,border:`1px solid ${C.border}`,padding:24}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14}}>
          ...
          <div>
            ...
            <div style={{display:"flex",gap:6}}>
              {[{id:"SMS",label:isAr?"رسالة نصية":"SMS"},{id:"Push",label:isAr?"إشعار فوري":"Push"},{id:"Both",label:isAr?"الكل":"Both"}].map(ch=><button key={ch.id} onClick={()=>setCompChannel(ch.id)}
                style={{flex:1,padding:"10px",borderRadius:10,border:`1px solid ${compChannel===ch.id?C.gold:C.border}`,background:compChannel===ch.id?C.goldLight:C.white,fontSize:13,fontWeight:600,color:compChannel===ch.id?C.goldDim:C.textMuted,cursor:"pointer"}}>{ch.label}</button>)}
            </div>
          </div>
        </div>
        <div style={{marginBottom:14}}>
      ...
      {tab==="bulk"&&<div style={{background:C.white,borderRadius:14,border:`1px solid ${C.border}`,padding:24}}>
        ...
        <Btn variant="gold" onClick={async ()=>{if(!compBody){showToast(isAr?"⚠️ الرسالة فارغة":"⚠️ Message is empty");return;}
          ...
          try{const recipients=bulkRecipients.map(i=>i.phone||i.nationalId).filter(Boolean);const r=await apiFetch("/sms/bulk",{method:"POST",body:JSON.stringify({recipients,message:compBody})});if(r&&r.ok){const d=await r.json();showToast(`✅ ${d.count||d.sent||count} ${isAr?"رسالة أُرسلت":"messages sent"}`);} else {showToast(isAr?"⚠️ خطأ":"⚠️ Error");}}catch(e){showToast(isAr?"⚠️ خطأ":"⚠️ Error");}
...
const Settings = ({ onLangChange }) => {
  const { t, isAr, commSplit, setCommSplit, gatewaySettings, setGatewaySettings, commissionRates, setCommissionRates, cancelFee, setCancelFee, storageFeeSettings, setStorageFeeSettings, reportingConfig, setReportingConfig } = useLang();
  const { gold: _sfGold, silver: _sfSilver, plat: _sfPlat } = useLivePrices();
  // Load storage fee config from API on mount
  useEffect(()=>{
    apiFetch("/storage-fees/config").then(r=>r&&r.ok?r.json():null).then(d=>{
      if(d) setStorageFeeSettings({
        goldAnnualPct: String(d.gold_rate_percent||"0.5"),
        silverAnnualPct: String(d.silver_rate_percent||"0.75"),
    ...
    apiFetch("/commission/rates").then(r=>r&&r.ok?r.json():null).then(d=>{
      if(d) setCommissionRates({buyer:String(d.buyer_rate||d.buyer||"2.0"),seller:String(d.seller_rate||d.seller||"1.0")});
    }).catch(()=>{});
    ...
    <div>
      ...
      {tab==="STORAGE FEES"&&<div>
        ...
        <div style={{display:"flex",justifyContent:"flex-end",marginTop:4}}>
          <Btn variant="teal" onClick={async ()=>{
              try {
                await apiFetch("/storage-fees/config",{method:"PUT",body:JSON.stringify({
                  gold_rate_percent: parseFloat(storageFeeSettings?.goldAnnualPct||"0.5"),
  ...
  return (
    <div style={{display:"flex",alignItems:"center",gap:5,background:bgColor,padding:"4px 10px",borderRadius:7,border:`1px solid ${color}55`,direction:"ltr"}}>
      ...
      <span style={{fontSize:12,color:color,fontWeight:800,fontVariantNumeric:"tabular-nums"}}>
        {(price||0).toLocaleString("en-SA",{minimumFractionDigits:2,maximumFractionDigits:2})}
      </span>
...
const runGlobalAML = ({investors, orders, matches, walletMovements, withdrawals, bars, blacklist, transactions, appointments}) => {
  const alerts = [];
  const now = new Date();
  const allTxns = transactions && transactions.length > 0 ? transactions : matches.map(m=>({id:m.id,buyerNationalId:m.buyerNid||"",sellerNationalId:m.sellerNid||"",buyerName:m.filledFor,sellerName:m.filledFor,total:String(m.totalSAR),metal:m.metal,status:"COMPLETED",date:m.date,type:"MATCH"}));
  ...
  investors.forEach(inv => {
    ...
    if(totalVol > 60000) push("R01",totalVol>200000?"HIGH":"MEDIUM","High-Value Trading Activity",`Total volume SAR ${(totalVol||0).toLocaleString()} exceeds SAMA threshold.`,"VOLUME");
    if(buyVol > 10000 && sellVol > 10000 && txFreq > 0.5) push("R02","HIGH","Rapid Buy-Sell Pattern",`Buy SAR ${(buyVol||0).toLocaleString()} / Sell SAR ${(sellVol||0).toLocaleString()}, freq ${txFreq.toFixed(2)}/day.`,"PATTERN");
    ...
    if(daysSinceJoin > 120 && totalVol > 30000) {
      ...
      if(recentTx.length >= 3 && olderTx.length === 0) push("R15","HIGH","Dormant Account Reactivation",`Account ${Math.ceil(daysSinceJoin)} days old, no prior activity but ${recentTx.length} TX in last 14 days totaling SAR ${(totalVol||0).toLocaleString()}. Sudden reactivation pattern.`,"BEHAVIOR");
    }
  ...
  if(matches.length>0){
    ...
    if(totalMatchVol>100000) alerts.push({rule:"R11",level:"MEDIUM",nid:"PLATFORM",name:"System",title:"High Platform Match Volume",detail:`SAR ${(totalMatchVol||0).toLocaleString()} across ${matches.length} trades.`,category:"SYSTEM",automatedAt:now.toISOString(),key:"R11:PLATFORM"});
  }
...
const runCMAManipulation = ({investors, orders, matches, blacklist, transactions}) => {
  ...
  openOrders.forEach(o1 => {
    openOrders.forEach(o2 => {
      ...
      if(o1.nationalId === "SYSTEM" || o1.nationalId === "7031990530" || o2.nationalId === "SYSTEM" || o2.nationalId === "7031990530") return;
  ...
  investors.forEach(inv => {
    ...
    if(turnoverRatio > 3) {
      push("CMA-06","MEDIUM",nid,inv.nameEn,
        ...
        `Turnover ratio ${turnoverRatio.toFixed(1)}x (SAR ${(totalTurnover||0).toLocaleString()} traded vs SAR ${(holdings||0).toLocaleString()} holdings). Art 16 prohibits trades contrary to client interest given frequency relative to portfolio.`,
        "Art 16","CHURNING");
    }
  });
      ...
      if(firstSell && firstSell > lastBuy) {
        ...
        if(daysBetween < 5 && buyVol > 20000 && sellVol > 10000) {
          push("CMA-07","HIGH",nid,inv.nameEn,
            ...
            `Bought SAR ${(buyVol||0).toLocaleString()} then sold SAR ${(sellVol||0).toLocaleString()} within ${Math.ceil(daysBetween)} days. May indicate promoting purchase for purpose of selling (Art 3(a)(2)).`,
...
export default function App() {
      ...
      const endpoints = [
        ...
        { path: "/appointments", setter: setAppAppointments, transform: (data) => {
          ...
          return items.map(a => ({
            ...
            qty: a.quantity || "", quantity: a.quantity || "", vault: a.vault_location || "Riyadh",
        ...
        { path: "/matches", setter: (data) => {
          setAppMatches(data);
          // Also populate transactions from matches
          setAppTransactions(data.map(m => ({
            id: m.id, _uuid: m._uuid || m.id,
            buyerNationalId: m.buyerNid || "",
            sellerNationalId: m.sellerNid || "",
            buyerName: m.buyerName || "",
            sellerName: m.sellerName || "",
            total: String(m.totalSAR || 0),
            metal: m.metal || "Gold", status: "COMPLETED",
            date: m.date || "",
            type: "MATCH",
          })));
        }, transform: (data) => {
        ...
        { path: "/blocks", setter: setAppBlocks, transform: (data) => {
          ...
          return items.map(b => ({
            ...
            timestamp: b.created_at || "", size: b.size_bytes ? (Number(b.size_bytes||0)/1048576).toFixed(2)+" MB" : "0 MB",
            quorumMet: b.quorum_met || false,
          }));
        }},
      ];

      // Fetch all endpoints in parallel for faster load
      const results = await Promise.allSettled(
        endpoints.map(ep => apiFetch(ep.path+(ep.path.includes("?")?"&":"?")+"page=1&page_size=9999").then(async resp => {
          if (resp && resp.ok) {
            const raw = await resp.json();
            const transformed = ep.transform(raw);
            if (Array.isArray(transformed)) { ep.setter(transformed); return true; }
          } else if (resp) {
            console.warn("[Tanaqul API] "+ep.path+" returned "+resp.status);
          }
          return false;
        }))
      );
  ...
  useEffect(() => {
    const handleLogout = () => {localStorage.removeItem("tanaqul_token");localStorage.removeItem("tanaqul_refresh");setLoggedIn(false);setCurrentAdmin("admin@tanaqul.sa");};
    window.addEventListener("tanaqul_logout", handleLogout);
    const handleForbidden = (e) => {
      // Show a non-blocking access denied notification
      console.warn("[Tanaqul] Access denied:", e.detail?.path);
    };
    window.addEventListener("tanaqul_forbidden", handleForbidden);
    return () => { window.removeEventListener("tanaqul_logout", handleLogout); window.removeEventListener("tanaqul_forbidden", handleForbidden); };
  }, []);
  ...
  const [mmAccount, setMMAccount] = useState({
    cash: 0, cashBalance: 0, gold:{g:0,grams:0,avg:0}, silver:{g:0,grams:0,avg:0}, platinum:{g:0,grams:0,avg:0},
  ...
  useEffect(() => {
    if(!loggedIn) return;
    // Build combined transactions list from live matches
  ...
  const appDataValue = {
    ...
    transactions: appTransactions, setTransactions: setAppTransactions,
  ...
  if (!loggedIn) return (
    <AppDataContext.Provider value={appDataValue}>
      <LangContext.Provider value={{ lang, t: tFn, isAr, switchLang, bidEnabled, setBidEnabled, tradingOpen, setTradingOpen, commSplit, setCommSplit, gatewaySettings, setGatewaySettings, commissionRates, setCommissionRates, cancelFee, setCancelFee, storageFeeSettings, setStorageFeeSettings, reportingConfig, setReportingConfig }}>
        <LoginPage onLogin={() => {
        setLoggedIn(true);
        try { const t=localStorage.getItem("tanaqul_token"); if(t){const p=JSON.parse(atob(t.split(".")[1]));setCurrentAdmin(p.email||p.sub||"admin@tanaqul.sa");} } catch(e){}
      }} />
