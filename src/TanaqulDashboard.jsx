...
function SAR({ size = "1em", color = "currentColor", style = {} }) {
  return (
    <svg viewBox="0 0 512 572" style={{ width: size, height: size, display: "inline-block", verticalAlign: "middle", flexShrink: 0, ...style }}>
      <path fill={color} d="M156.59 375.86c-3.42 0-7.11.57-11.07 1.7-3.97 1.13-7.73 2.94-11.3 5.41-3.58 2.47-6.53 5.7-8.84 9.68-2.31 3.98-3.47 8.77-3.47 14.37 0 8.23 2.88 14.31 8.62 18.25 5.75 3.93 12.41 5.9 19.98 5.9 6.01 0 11.95-1.27 17.83-3.82 5.87-2.55 11.24-5.93 16.1-10.17 4.85-4.23 8.95-9.14 12.29-14.72 3.34-5.59 5.43-11.33 6.25-17.24-4.48-3.42-10.06-5.87-16.72-7.38-6.67-1.51-13.17-2.37-19.5-2.37l-10.17.39z"/>
      <path fill={color} d="M475.87 234.97c-7.44-16.55-17.75-30.8-30.95-42.77-13.2-11.96-28.67-21.22-46.43-27.77-17.75-6.55-36.88-9.83-57.39-9.83-15.59 0-30.37 2.04-44.35 6.12-13.99 4.08-26.78 9.81-38.38 17.19-11.6 7.38-21.67 16.24-30.22 26.58-8.55 10.34-15.2 21.67-19.95 33.99l-88.56-2.94c5.14-12.32 11.5-24.37 19.09-36.15 7.58-11.78 16.68-22.56 27.31-32.34 10.62-9.78 22.8-17.96 36.54-24.55 13.73-6.59 29.26-11.07 46.57-13.46-10.59-11.73-23.31-21.6-38.16-29.58-14.86-7.99-30.51-14.04-46.96-18.16-16.45-4.12-33.07-6.18-49.84-6.18-25.96 0-50.14 4.62-72.55 13.85-22.42 9.24-41.92 22.12-58.53 38.65-16.6 16.53-29.6 36.22-39.01 59.08-9.41 22.86-14.12 47.9-14.12 75.14 0 27.24 4.71 52.28 14.12 75.13 9.41 22.86 22.42 42.56 39.01 59.08 16.6 16.53 36.1 29.41 58.53 38.65 22.42 9.24 46.59 13.85 72.55 13.85 16.77 0 33.38-2.06 49.84-6.17 16.45-4.12 32.1-10.17 46.96-18.17 14.86-7.99 27.57-17.85 38.16-29.58 10.59-11.73 18.72-25.14 24.38-40.23l87.47-6.85c-5.14 20.31-13.47 39.26-24.99 56.86-11.52 17.61-25.49 32.96-41.92 46.08-16.44 13.12-34.87 23.4-55.3 30.84-20.43 7.44-42.26 11.16-65.49 11.16-33.65 0-64.84-6.18-93.58-18.55-28.75-12.37-53.64-29.41-74.69-51.14-21.05-21.73-37.54-47.29-49.49-76.7-11.95-29.41-17.93-61.18-17.93-95.3s5.98-65.89 17.93-95.3c11.95-29.41 28.44-54.97 49.49-76.7 21.05-21.73 45.94-38.77 74.69-51.14 28.74-12.37 59.93-18.55 93.58-18.55 23.24 0 45.07 3.72 65.49 11.16 20.43 7.44 38.87 17.72 55.3 30.84 16.44 13.12 30.41 28.48 41.92 46.08 11.52 17.61 19.85 36.55 24.99 56.86l-87.47 6.85c-1.9-7.87-4.75-15.43-8.55-22.7z"/>
    </svg>
  );
}
...
const Financials = () => {
  ...
  const txRows = matches.map(m=>({id:m.id,investor:m.filledFor,type:"MATCH",metal:m.metal,metalAmt:String(m.totalSAR),commission:String(m.commission),adminFee:String(m.adminFee||0),method:"Wallet",total:String(m.totalSAR),status:"COMPLETED",date:m.date})).concat(MOCK.transactions);
...
const Blocks = () => {
  ...
  const blockTxRows = matches.map(m=>({id:m.id,investor:m.filledFor,type:"MATCH",metal:m.metal,metalAmt:String(m.totalSAR),commission:String(m.commission),adminFee:String(m.adminFee||0),method:"Wallet",total:String(m.totalSAR),status:"COMPLETED",date:m.date})).concat(MOCK.transactions);
  return (
    <div>
      ...
      <div style={{background:C.navyDark,borderRadius:12,padding:"12px 16px",marginBottom:18,display:"flex",alignItems:"center",gap:14,flexWrap:"wrap"}}>
        ...
        <span style={{color:C.silverText}}>|</span>
        <span style={{fontSize:13,color:"#A89880",fontFamily:"monospace"}}>Last: {MOCK.stats.lastBlock}</span>
      </div>
    ...
    <div style={{display:"flex",alignItems:"center",gap:7,flexWrap:"nowrap"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:0,background:C.goldLight,borderRadius:7,border:`1px solid ${C.gold}44`,overflow:"hidden",direction:"ltr"}}>
        <div style={{padding:"4px 9px",display:"flex",alignItems:"center",gap:4}}>
          ...
          <span style={{fontSize:11,color:C.goldDim,fontWeight:700}}>#{MOCK.stats.blockNumber}</span>
        </div>
